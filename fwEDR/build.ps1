...
<#
.SYNOPSIS  Cross-compile EDR for all supported platforms.
.EXAMPLE
...
if ($Clean) {
    ...
    Write-Host "$OutputDir/ removed." -ForegroundColor Green; exit 0
}

Write-Host "================================" -ForegroundColor Cyan
Write-Host "EDR - Build Script"              -ForegroundColor Cyan
Write-Host "================================" -ForegroundColor Cyan
Write-Host ""

if (-not (Get-Command go -ErrorAction SilentlyContinue)) {
    Write-Host "ERROR: Go not found. Install from https://go.dev/dl/" -ForegroundColor Red; exit 1
}
...
function Build-Platform {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) { Write-Host "  [FAILED]" -ForegroundColor Red; Write-Host $out -ForegroundColor Red; return $false }
        $sizeMB = [math]::Round((Get-Item $outputPath).Length / 1MB, 1)
        Write-Host "  [OK] $binary ($sizeMB MB)" -ForegroundColor Green
        Copy-Item "edr.yaml" "$platformDir\edr.yaml" -Force
        Write-Host "  [OK] Copied edr.yaml" -ForegroundColor Green
        return $true
...
switch ($Platform) {
    ...
    "macos"   { $ok = (Build-Platform "darwin" "amd64") -and (Build-Platform "darwin" "arm64") }
    "all" {
...
Write-Host ""
if ($ok) { Write-Host "Build complete! Output: $OutputDir\" -ForegroundColor Green; exit 0 }
else     { Write-Host "Some builds failed." -ForegroundColor Red; exit 1 }
